...
**Implications:** Future popup/Options changes must hold the AA bar (see CLAUDE.md). Shipped to `main` (`f50ab43`) and released to users in **v1.2.0**: the version bump, `npm run store:assets` regen, and store package were prepped in `717cc2d`, the `v1.2.0` GitHub release was cut, and the Chrome Web Store listing is now live at v1.2.0. Firefox Add-ons remains the one outstanding manual submission (the v1.2.0 source archive + checklist are ready under `store/`).
