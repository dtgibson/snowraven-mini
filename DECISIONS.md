...
## Accessibility: WCAG 2.1 AA pass on the popup + Options — 2026-06-14

**Decision:** Bring the popup and Options UI to WCAG 2.1 AA and keep it there. An in-session multi-agent audit found 15 issues (0 critical / 2 serious / 6 moderate / 7 minor), all fixed: a dark-theme hover-contrast token, screen-reader announcements for every resolved popup state (routed through the shared polite live region), `<main>`/`<header>`/`<h1>` landmarks and an h2 hierarchy on both surfaces, labelled keyboard-reachable Weather/Tide regions, a darker light-theme input border, zoom-safe inputs, and arrow glyphs hidden from accessible names.

**Rationale:** The popup was already substantially accessible, so closing the remaining gaps is UI-quality maintenance, not a new feature — it stayed in the Improve lane. The weather/tide block is made keyboard-reachable with `tabindex` rather than rewrapped: the byte-for-byte SnowRaven parity contract is on the *copied* string (golden-tested), which CSS wrapping wouldn't change, but `tabindex` also keeps the on-screen block identical to the desktop. Presentation/ARIA only — no logic, dependency, or permission changes; the golden parity tests stayed green throughout.

**Implications:** Future popup/Options changes must hold the AA bar (see CLAUDE.md). Shipped to `main` (`f50ab43`) and released to users in **v1.2.0**: the version bump, `npm run store:assets` regen, and store package were prepped in `717cc2d`, the `v1.2.0` GitHub release was cut, and both the Chrome Web Store and Firefox Add-ons listings are now live at v1.2.0, at full cross-store parity (Firefox reached v1.2.0 on 2026-06-15) — see the next entry.

## Firefox Add-ons live at v1.2.0 — cross-store parity — 2026-06-15

**Decision:** The Firefox Add-ons (AMO) listing is live at <https://addons.mozilla.org/firefox/addon/snowraven-mini/>, now serving **v1.2.0**. The cross-store launch landed in two steps: it first published at **v1.1.0** on 2026-06-14 (the website's Firefox button swapped from "coming soon" to a live link matching the Chrome button, and the README and `docs/HELP.md` install sections were pointed at it), then was updated to **v1.2.0** on 2026-06-15, reaching full parity with the Chrome Web Store. SnowRaven Mini is now live at v1.2.0 on both major stores — the Chrome Web Store (Chrome / Edge / other Chromium) and Firefox Add-ons — and the v1.2.0 WCAG 2.1 AA accessibility pass is in users' hands on both.

**Rationale:** The store-submission and Chrome-launch decisions always planned to swap the Firefox "coming soon" button for a live link once AMO published (see the prior Chrome entry's implications). AMO approved the package — the `gecko.data_collection_permissions: { required: ["none"] }` declaration satisfied the data-collection review, and the source archive plus `BUILD.md` satisfied the reproducible-build review. One codebase, one manifest, one build serves both stores, so the v1.2.0 Firefox update was a straight re-submission of the same package that already shipped to Chrome.

**Implications:** Every store release now ships to both the Chrome Web Store and Firefox Add-ons in lockstep (regenerate `store/` assets and zips, bump the version, upload to each — `store/firefox-submit-checklist.md` covers the Firefox side). Keep the AMO `gecko.data_collection_permissions` declaration in the manifest — AMO requires it for new submissions and updates.

## Combined weather & tide copy: manual-only, additive, port-and-lock — 2026-06-15

**Decision:** The "Copy weather & tide together" button copies SnowRaven desktop's `buildCombined()` output byte-for-byte and is **manual-only and additive**. It appears only when both blocks resolve, sits alongside (never replaces) the per-block Copy buttons, and never auto-fires — the popup keeps auto-copying weather alone on open, exactly as before. It is a port-and-lock of the already-present `buildCombined` (`src/lib/tideFormatter.ts`), not a re-implementation: the lane wired the existing function to a button and threaded the tide `body` through the UI state, with parity locked by a golden `.toBe` test.

**Rationale:** SnowRaven never auto-copies its combined output, so neither does Mini — auto-firing the combined block would break the founding "weather copies on click; tide is yours to add" posture and risk pasting tide a user didn't want. Additive keeps the existing one-click habit untouched. Re-deriving `buildCombined` would invite byte drift against the desktop app, whose exact-match output is the product's whole premise; porting-and-locking keeps the one source of truth and fails the build on any drift.

**Implications:** The combined copy is built from the tide `body` (which keeps the inline NOAA credit), never the standalone `formatted` block (which appends `· via SnowRaven`) — combining from `formatted` would double the attribution. If SnowRaven changes `buildCombined`, re-sync and re-lock the golden. Future copy paths stay manual unless there is a deliberate, logged reason to auto-fire.

## Popup width: a hard 380px floor — 2026-06-28

**Decision:** Give the popup shell (`.sr-popup`) a hard `min-width: 380px` floor and drop its `max-width: 100%`. This fixes a latent bug where, after a lookup, the popup could narrow until it collapsed to an unreadable vertical bar — on one Mac but not another. Shipped to `main` (`ad2ee5f`) as **v1.4.0**.

**Rationale:** An MV3 popup window auto-sizes to its content, so `.sr-popup`'s `width: 380px; max-width: 100%` had no stable basis: the percentage `max-width` resolves against a containing block whose width the browser derives from the content, with no hard pixel floor anywhere on `.sr-popup`/`#root`/`body`/`html`. The result state is the only one tall enough to summon a vertical scrollbar; on a Mac set to "Show scroll bars: Always" (classic, width-reserving) that scrollbar shaved ~15px off the width, dropped the viewport below 380, and the popup auto-sizer ratcheted the width down toward min-content — the vertical sliver. The other Mac uses overlay scrollbars (0px reserved), so it never triggered — same build, different macOS setting. The fix lives entirely at the popup-container width layer: the weather/tide `<pre>` (its `white-space: pre`, `overflow-x: auto`, `tabIndex`, and text) is untouched, so the byte-for-byte SnowRaven parity contract and the golden tests are unaffected. Latent since v0.1.0; no app JS is involved (there is no resize/measure code — confirmed by grep). The root cause was confirmed by a multi-angle diagnosis and an adversarial review.

**Implications:** Do not re-add a percentage `max-width` to `.sr-popup` — the inline CSS comment guards this; the popup is a fixed 380px with a hard floor. The Options page is unaffected (it uses `.sr-options-*` and opens in a real tab with a fixed viewport). Any future popup-width change must keep a hard pixel floor so the auto-sized window can never collapse below it.
