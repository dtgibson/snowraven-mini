...
**Implications:** Future popup/Options changes must hold the AA bar (see CLAUDE.md). Shipped to `main` (`f50ab43`); a Chrome Web Store release of this is deferred to the next version bump (v1.2.0 + store-asset regen + manual upload), so the live store extension stays at v1.1.0 until then.
