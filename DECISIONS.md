...
**Implications:** Future popup/Options changes must hold the AA bar (see CLAUDE.md). Shipped to `main` (`f50ab43`) and released to users in **v1.2.0**: the version bump, `npm run store:assets` regen, and store package were prepped in `717cc2d`, the `v1.2.0` GitHub release was cut, and the Chrome Web Store listing is now live at v1.2.0. Firefox Add-ons is live at v1.1.0; bringing it to v1.2.0 is a pending manual submission — see the next entry.

## Firefox Add-ons live (v1.1.0); v1.2.0 Firefox submission pending — 2026-06-14

**Decision:** The Firefox Add-ons (AMO) listing is live at <https://addons.mozilla.org/firefox/addon/snowraven-mini/>, currently serving **v1.1.0**. The website's Firefox button is now a live link to the listing (matching the Chrome button), and the README and `docs/HELP.md` install sections point at it. SnowRaven Mini now has a presence on both major stores — the Chrome Web Store (Chrome / Edge / other Chromium, at v1.2.0) and Firefox Add-ons (v1.1.0) — but Firefox trails Chrome by one version: the v1.2.0 accessibility pass is not yet on Firefox. Updating the Firefox listing to **v1.2.0** is the outstanding manual step; the v1.2.0 Firefox zip + source archive are prepared under `store/`.

**Rationale:** The store-submission and Chrome-launch decisions always planned to swap the Firefox "coming soon" button for a live link once AMO published (see the prior Chrome entry's implications). AMO approved the v1.1.0 package — the `gecko.data_collection_permissions: { required: ["none"] }` declaration satisfied the data-collection review, and the source archive plus `BUILD.md` satisfied the reproducible-build review. One codebase, one manifest, one build still serves both stores, so the v1.2.0 update is a straight re-submission of the already-prepared package.

**Implications:** First, bring Firefox to parity by submitting the prepared v1.2.0 package via `store/firefox-submit-checklist.md`. Going forward, every store release must upload to both the Chrome Web Store and Firefox Add-ons (regenerate `store/` assets and zips, bump the version, upload to each). Keep the AMO `gecko.data_collection_permissions` declaration in the manifest — AMO requires it for new submissions and updates.
