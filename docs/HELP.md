...
**Night checklists.** When the checklist is at night (any sampled hour falls outside its sunrise-to-sunset window), a moon-phase emoji is appended directly to the condition emoji as a single run with no space between them, for example `☁️🌗`. The phase comes from the checklist's first hour and is mirrored in the Southern Hemisphere. Daytime checklists show the condition emoji alone. This matches the SnowRaven desktop app.
