...
## Distribution
GitHub: <https://github.com/dtgibson/snowraven-mini> — source on `main`, now at **v1.1.0** after the moon-phase parity update. The **v1.0.0** GitHub release still carries the Chrome, Firefox, and source zips; cutting a 1.1.0 release (regenerated zips/assets, website version pill) is a pending step. The full store listing package (copy, assets, disclosures, submit checklists) is prepared under `store/` at v1.0.0 artifact names. Not yet submitted to the Chrome Web Store or Firefox Add-ons; the developer-account setup, the one-time $5 Chrome fee, and the uploads are the maintainer's manual step, guided by the checklists in `store/`.
