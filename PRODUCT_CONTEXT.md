...
## Distribution
GitHub: <https://github.com/dtgibson/snowraven-mini> — source on `main` at **v1.2.0**, with the **v1.2.0** release carrying the Chrome, Firefox, and source zips. **Live on the Chrome Web Store** ([listing](https://chromewebstore.google.com/detail/snowraven-mini/dfbphfbhbehdlfepoigechmjbifpndhc)) as v1.2.0; Microsoft Edge and other Chromium browsers install from that same listing after enabling Edge's off-by-default "Allow extensions from other stores" (documented in `docs/HELP.md`). Also **live on Firefox Add-ons** (<https://addons.mozilla.org/firefox/addon/snowraven-mini/>) at v1.1.0 — one version behind Chrome; the prepared v1.2.0 update (source archive plus `BUILD.md` satisfy AMO's reproducible-build review) is pending submission, and the submit checklists live under `store/`. The full store listing package (copy, assets, disclosures, checklists) is prepared under `store/`.
