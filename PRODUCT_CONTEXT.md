...
- **Landing website** — *shipped.* A static marketing site under `website/`, mirroring the SnowRaven app site, deployed to GitHub Pages at `snowravenmini.dtgibson.com`. Hero, a privacy band, feature rows that render the live popup UI, an install section with "coming soon" store buttons and the GitHub-release path, and a footer. Hand-authored HTML/CSS/JS, system fonts, no build, light/dark. Live now at <https://dtgibson.github.io/snowraven-mini/>; the custom domain awaits a DNS record. The stores stay "coming soon" until the listings publish.
...
A landing site lives in `website/` and is deployed to GitHub Pages (live at <https://dtgibson.github.io/snowraven-mini/>; custom domain `snowravenmini.dtgibson.com` pending a DNS CNAME to `dtgibson.github.io`).
